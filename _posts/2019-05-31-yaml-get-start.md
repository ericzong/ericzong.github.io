...
* 注释（只有行注释）以 `#` 开始，且 `#` 之前至少要有一个空白字符。

* `:` 后有一空格。

* 列表项用 `-` 列出，或者列表用流式（flow）语法 `[]` 列出并用 `,` 分隔。

* 映射项用 `: ` 列出，或者映射用流式语法 `{}` 列出并用 `,` 分隔。如果键有 `?` 前缀，则可以是复杂类型。
...
```yaml
int: 0b1010_0101 # 二进制表示
octal: 0o52             # 八进制表示
hex: 0xFF               # 十六进制表示

float: 1.23e+6          # 浮点数，科学计数法
fixed: 42.5             # 固定小数
minmin: -.inf           # 表示负无穷
notNumber: .NaN         # 无效数字

null:                   # 空值
boolean: [true, false, TRUE, FALSE]  # 布尔值
string: 'text'          # 字符串

date: 2018-07-30
canonical: 2018-07-30T00:00:00.0Z
iso8601: 2018-07-30T08:00:00.00+08:00
